  ...
  const alerts = [
    {
      type: "Vencimiento",
      icon: CalendarClock,
      items: conditionants.filter((c) => c.status === "vencida"),
    },
    {
      type: "Crítica sin evidencia",
      icon: AlertTriangle,
      items: conditionants.filter((c) => c.risk === "crítico" && c.status === "no cumple"),
    },
    {
      type: "Próximo reporte",
      icon: Bell,
      items: conditionants.filter((c) => c.categoryCode === "C10").slice(0, 3),
    },
  ];
  return (
    <>
      <PageHeader
        ...
        description="Notificaciones automáticas por plazo, periodicidad, falta de evidencia o incidencia."
      />
