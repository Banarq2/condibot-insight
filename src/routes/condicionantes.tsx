...
function Conditionants() {
  ...
  const filtered = conditionants.filter(
    (c) =>
      (cat === "all" || c.categoryCode === cat) &&
      (q === "" ||
        c.summary.toLowerCase().includes(q.toLowerCase()) ||
        c.originalText.toLowerCase().includes(q.toLowerCase())),
  );
          ...
          <CardContent className="flex flex-wrap items-center gap-3 p-4">
            <div className="relative min-w-[260px] flex-1">
              ...
              <Input
                ...
                placeholder="Buscar texto, palabra clave..."
                className="pl-9"
              />
            </div>
            ...
            <p className="text-sm text-muted-foreground">
              {filtered.length} de {conditionants.length}
            </p>
          </CardContent>
        </Card>
