...
export const Route = createFileRoute("/auth")({
  head: () => ({ meta: [{ title: "Acceso — VMO Sustentabilidad" }] }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  const handleGoogle = async () => {
    ...
    if (result.error) { toast.error("Error con Google: " + result.error.message); setLoading(false); }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-primary/10 via-background to-info/10 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="mt-3 text-2xl">VMO Sustentabilidad</CardTitle>
          <CardDescription>Validador de Condicionantes Ambientales</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="signin">
            ...
            <TabsContent value="signin">
              <form onSubmit={handleSignIn} className="space-y-3 pt-4">
                <div><Label>Email corporativo</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></div>
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignUp} className="space-y-3 pt-4">
                ...
                <div><Label>Email corporativo</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></div>
                <div><Label>Contraseña</Label><Input type="password" minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} required /></div>
          ...
          <div className="relative my-4"><div className="absolute inset-0 flex items-center"><span className="w-full border-t" /></div><div className="relative flex justify-center text-xs"><span className="bg-card px-2 text-muted-foreground">o</span></div></div>
