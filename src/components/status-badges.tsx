...
export function RiskBadge({ risk }: { risk: RiskLevel }) {
  ...
  return <span className={cn("inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium capitalize", map[risk])}>{risk}</span>;
}
...
export function StatusBadge({ status }: { status: Status }) {
  ...
  return <span className={cn("inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium capitalize", map[status])}>{status}</span>;
}

export function CategoryBadge({ code }: { code: string }) {
  return <span className="inline-flex items-center rounded-md border border-primary/30 bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">{code}</span>;
}
